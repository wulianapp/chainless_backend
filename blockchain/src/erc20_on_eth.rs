...
impl EthContractClient<Erc20> {
    ...
    pub async fn balance_of(&self, symbol: &str, address: &str) -> Result<u128> {
    ...
    pub async fn allowance(&self, symbol: &str, owner: &str, spender: &str) -> Result<u128> {
        let owner = Address::from_str(owner).unwrap();
        let spender = Address::from_str(spender).unwrap();
        let coin_ca = Address::from_str(symbol).unwrap();
...
mod tests {
    ...
    async fn test_erc20_op() {
        ...
        let _approve_res = cli
            .relayer_approve(usdt_addr, &spender, amount)
            .await
            .unwrap();
        let allow_amount = cli
            .allowance(usdt_addr, relayer_addr, &spender)
            .await
            .unwrap();
