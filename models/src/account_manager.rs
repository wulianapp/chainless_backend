...
use ethers_core::types::{Res, U256};
use postgres::Row;
//#[derive(Serialize)]
use serde::Serialize;
...
pub fn single_insert(data: UserInfo) -> Result<(),String>{
    ...
    let UserInfo {phone_number,email,state,
        multi_sign_strategy,verified,pwd_hash,invite_code,direct_invited_number,
        ancestors,points,grade,fans_num} = data;
    //todo: convert ancestors text array
    let mut sql = format!("insert into users (phone_number,email,state,\
    multi_sign_strategy,verified,pwd_hash,invite_code,direct_invited_number,\
...
pub fn update_password(new_password:&str,filter:UserFilter) {
    let sql = format!("UPDATE users SET pwd_hash='{}' where {}", new_password, filter.to_string());
