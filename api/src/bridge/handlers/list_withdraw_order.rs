...
pub(crate) async fn req(req: HttpRequest,request_data: ListWithdrawOrderRequest) -> BackendRes<Vec<ListWithdrawOrderResponse>> {
    ...
    for (id,info) in order_ids_on_chainless {
        ...
        let signatures = info.signers.into_iter().map(|x| x.signature.unwrap_or("".to_string())).collect();
