...
pub(crate) async fn req(req: HttpRequest) -> BackendRes<Vec<(String,Vec<CoinTxView>)>> {
    ...
    let predecessor = get_user(UserFilter::ById(user_id))?
        .ok_or(AccountManagerError::UserIdNotExist)?;

    println!("searchMessage user_id {}", user_id);
    let message = predecessor
        .user_info.account_ids
        .iter()
        .map(|acc_id|
            {
                let acc_msg = models::coin_transfer::get_transactions(
                    CoinTxFilter::ByAccountPending(acc_id.to_string())).unwrap();
...
pub(crate) async fn req_by_account_id(req: HttpRequest,request_data: searchMessageByAccountIdRequest) -> BackendRes<Vec<CoinTxView>> {
    let user_id =
        token_auth::validate_credentials(&req)?;
    let predecessor = get_user(UserFilter::ById(user_id))?
        .ok_or(AccountManagerError::UserIdNotExist)?;
    ...
    println!("searchMessage user_id {}", user_id);
    let acc_msg = models::coin_transfer::get_transactions(
        CoinTxFilter::ByAccountPending(request_data.account_id)).unwrap();
    Ok(Some(acc_msg))
}